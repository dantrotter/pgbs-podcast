<template>
		<main>
				...
				<a class="doc-link" :href="outlines[friendlyName]" target="_blank">
					<svg class="word-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 96 96" width="25" height="25" fill="#FFF" stroke-miterlimit="10" stroke-width="2">
					...
					</svg>Download outlines for {{ friendlyName }}
				</a>
				<ol class="book-list">
						<li v-for="chapter in audio[friendlyName]" :key="chapter">
							<h2 class="small-header">Chapter {{ chapter.chapter }}</h2>
							<ul>
								<li v-for="study in chapter.studies" :key="study.title">
									<a :href="study.audio" target="_blank">{{ study.title }}</a>
								</li>
							</ul>
						</li>
				</ol>
